...
function readOrCreateSessionId(): string {
  ...
  try { localStorage.setItem(CHAT_SESSION_KEY, id); } catch {}
  return id;
}

export interface ChatPanelProps {
  /** page = full /chat layout; drawer = compact slide-over */
  variant?: 'page' | 'drawer';
  /** Hide outer chrome when drawer provides its own header */
  hideHeader?: boolean;
}

export default function ChatPanel({ variant = 'page', hideHeader = false }: ChatPanelProps) {
  ...
  const padX = isDrawer ? 20 : 56;
  const headerH = hideHeader ? 0 : isDrawer ? 56 : 64;
  const footerH = isDrawer ? 120 : 172;

  return (
    <div
      style={{
        position: 'relative',
        height: isDrawer ? '100%' : 'calc(100dvh / 0.9 - 97px)',
        overflow: 'hidden',
        maxWidth: isDrawer ? 'none' : 1040,
        margin: isDrawer ? 0 : '0 auto',
      }}
    >
      {/* ------------------------------------------------------------------ */}
      {/* Header                                                               */}
      ...
      <div
        style={{
          ...
          right: 0,
          overflowY: 'auto',
          padding: isDrawer ? `20px ${padX}px` : '32px 56px',
          display: 'flex',
      ...
      <footer
        style={{
          ...
          padding: isDrawer ? `16px ${padX}px` : '24px 56px',
          display: 'flex',
          flexDirection: 'column',
          gap: isDrawer ? 12 : 16,
        }}
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 12, flexWrap: isDrawer ? 'wrap' : 'nowrap', justifyContent: 'center' }}>
